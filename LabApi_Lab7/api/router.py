...
@router.get("/", response_model=BookCursorPage, dependencies=[Depends(check_rate_limit)])
...
@router.get("/{book_id}", response_model=BookSchema, dependencies=[Depends(check_rate_limit)])
async def read_book(
    book_id: int,
    db: AsyncSession = Depends(get_db),
    _: str = Depends(get_current_user),
...
@router.post("/", response_model=BookSchema, status_code=201, dependencies=[Depends(check_rate_limit)])
...
@router.put("/{book_id}", response_model=BookSchema, dependencies=[Depends(check_rate_limit)])
...
@router.delete("/{book_id}", status_code=204, dependencies=[Depends(check_rate_limit)])
